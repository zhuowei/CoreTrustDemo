...
true && openssl req -newkey rsa:2048 -nodes -keyout root_key.pem -x509 -days 3650 -out root_certificate.pem \
	-subj "/C=CA/O=Worth Doing Badly/OU=Worth Doing Badly Certification Authority/CN=Worth Doing Badly iPhone Root CA" \
...
true && openssl req -newkey rsa:2048 -nodes -keyout codeca_key.pem -out codeca_certificate.csr \
	-subj "/C=CA/O=Worth Doing Badly/OU=Worth Doing Badly Certification Authority/CN=Worth Doing Badly iPhone Certification Authority" \
	-addext "1.2.840.113635.100.6.2.18=DER:0500" \
...
true && openssl req -newkey rsa:2048 -nodes -keyout dev_key.pem -out dev_certificate.csr \
	-subj "/C=CA/O=Worth Doing Badly/OU=Worth Doing Badly Certificate Authority/CN=Worth Doing Badly iPhone OS Application Signing" \
...
true && /usr/bin/openssl pkcs12 -export -in dev_certificate.pem -inkey dev_key.pem -certfile certificate_chain.pem \
	...
	-out dev_certificate.p12 -name "Worth Doing Badly iPhone OS Application Signing"
